...
const TrangChuyenDoi = ({ nguoiDung }) => {
  ...
  return (
    <div 
      className="min-h-screen bg-gradient-animated pb-12 px-4"
      style={{ paddingTop: 'calc(5rem + var(--safe-area-top, env(safe-area-inset-top, 0px)))' }}
    >
              ...
              <AnimatePresence mode="wait">
                ...
                {trangThaiXuLy === 'loi' && (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <motion.div
                        className={`w-14 h-14 rounded-xl flex items-center justify-center shrink-0 ${laLoiThieuToken ? 'bg-amber-500/20' : 'bg-red-500/20'
                          }`}
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                      >
