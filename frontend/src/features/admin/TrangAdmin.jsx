...
const TrangAdmin = () => {
  ...
  const taiDuLieu = useCallback(async () => {
    setDangTai(true)
    try {
      const [overviewRes, usersRes, historyRes, auditRes, paymentsRes] = await Promise.all([
        layTongQuanAdmin(),
        layDanhSachNguoiDungAdmin(),
          ...
          {activeTab === 'cau-hinh' && (
            <div className="max-w-4xl space-y-6">
              <div className="rounded-xl border border-white/5 bg-white/[0.02] p-5 shadow-sm">
                ...
                <p className="mb-6 text-sm text-slate-400">Điều chỉnh các cấu hình hoạt động của nền tảng (Frontend Mock UI).</p>
                
                <div className="space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="space-y-1.5">
                      <label className="text-sm font-medium text-slate-300">Giá Token tối thiểu (VNĐ)</label>
                      <input type="number" defaultValue={1} className="w-full rounded-lg border border-white/10 bg-slate-900 px-4 py-2 text-white placeholder:text-slate-600 focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500" />
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input type="number" defaultValue={60} className="w-full rounded-lg border border-white/10 bg-slate-900 px-4 py-2 text-white placeholder:text-slate-600 focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500" />
                      <p className="text-xs text-slate-500">Số lượng Token tương đương số trang được dùng miễn phí.</p>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-sm font-medium text-slate-300">SePay API Key</label>
                      <input type="password" defaultValue="************************" disabled className="w-full rounded-lg border border-white/5 bg-slate-900/50 px-4 py-2 text-slate-500 cursor-not-allowed" />
                      <p className="text-xs text-slate-500">Đã thiết lập an toàn qua môi trường Backend.</p>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-sm font-medium text-slate-300">Giới hạn file Upload lớn nhất (MB)</label>
                      <input type="number" defaultValue={20} className="w-full rounded-lg border border-white/10 bg-slate-900 px-4 py-2 text-white placeholder:text-slate-600 focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500" />
                      <p className="text-xs text-slate-500">Ngăn người dùng lợi dụng tài nguyên máy chủ.</p>
                    </div>
                  </div>
                  
                  <div className="pt-4 border-t border-white/10 flex justify-end">
                    <button className="rounded-lg bg-cyan-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-cyan-500 transition-colors shadow-lg shadow-cyan-500/20" onClick={() => toast.success('Lưu cấu hình thành công (Bản thử nghiệm)')}>
