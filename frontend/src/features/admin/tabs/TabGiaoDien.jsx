...
const TabGiaoDien = () => {
  ...
  const themes = [
    { id: 'dark-indigo', name: 'Dark Indigo', description: 'Mặc định, sang trọng với tông màu tím indigo' },
    { id: 'midnight-purple', name: 'Midnight Purple', description: 'Tông màu đen tuyền kết hợp tím neon' },
  ...
  return (
    <div className="rounded-xl border border-white/5 bg-white/[0.02] p-5 shadow-sm space-y-6">
      <h3 className="mb-1 text-lg font-semibold text-white flex items-center gap-2">
        <Palette className="h-5 w-5 text-purple-400" /> Quản lý Giao diện Toàn hệ thống
      </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {themes.map(t => (
          <div 
            ...
            className={`cursor-pointer rounded-xl border p-4 transition-all duration-300 ${theme === t.id ? 'bg-purple-500/10 border-purple-400 shadow-[0_0_15px_rgba(168,85,247,0.2)]' : 'border-white/10 bg-slate-900/50 hover:bg-slate-800'}`}
          >
            <div className="h-24 w-full rounded-md mb-4 flex divide-x divide-white/10 overflow-hidden shadow-inner uppercase font-bold text-[10px] text-white/50 tracking-wider">
              ...
              {t.id === 'midnight-purple' && <><div className="flex-1 bg-black p-2">Bg</div><div className="flex-1 bg-purple-500 p-2">Accent</div></>}
            ...
            <div className="mt-4 flex items-center justify-between">
              <span className={`text-xs font-medium px-2 py-1 rounded-md ${theme === t.id ? 'bg-purple-400 text-slate-900' : 'bg-white/5 text-slate-400'}`}>
